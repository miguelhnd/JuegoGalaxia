...
public class EscenarioSol extends JPanel implements ActionListener, KeyListener
{
    // Atributos 
    ...
    // Constructores
    public EscenarioSol(JFrame marco, int habi, boolean conquistado)
    {
        this.marco=marco;
        this.y=0;
        this.x=0;
        Icono=new ImageIcon("Imagenes/FondoSol.jpg");
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        { 
                ...
                for(int i=0; i<Enemigos.length; i++)
                {
                    ...
                    for(int j=0; j<BalaHumano.length; j++)
                    {
                       if(BalaHumano[j].detectar(Enemigos[i].getRectangle()))
                       {
                           ...
                           if(this.VidasEnemigas==0)
                           {
                               JOptionPane.showMessageDialog(null,"El Sol ha sido conquistado. Has ganado 10 puntos.");
                               Puntajes.setConquistas(1);
                               Puntajes.setPuntaje(10);
                               new Principal(this.marco,2,"PlanetaSol"); 
                               comprobar=false;
                           }
                       }
            ...
            {
                JOptionPane.showMessageDialog(null,"El sol esta deshabitado. Has ganado 10 puntos.");
                Puntajes.setConquistas(1);
                Puntajes.setPuntaje(10);
                new Principal(this.marco,2,"PlanetaSol"); 
                comprobar=false;
